...
class TabLayoutScrollView: UIView {
    ...
    func drawHeaderView() -> Void {
        ...
        while i<numberOfTabs {
            ...
            button.setTitleColor(nonSelectedTabTextColor, for: UIControl.State())
            button.setTitle(btnTitle, for: UIControl.State())
            button.addTarget(self, action: #selector(TabLayoutScrollView.buttonAction(_:)), for: UIControl.Event.touchUpInside)
        ...
        if scrollView != nil {
            for view in scrollView!.subviews {
                if view.isKind(of: UIButton.self) {
                    ...
                    if btn.tag == tag {
                        btn.setTitleColor(selectedTabTextColor, for: UIControl.State())
                        btn.titleLabel?.font = selectedTabTextFont
                    }
                    else {
                        btn.setTitleColor(nonSelectedTabTextColor, for: UIControl.State())
